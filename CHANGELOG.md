...
- Proxy endpoint `/api/og-image/:id` serves OG images from our domain (avoids cross-domain issues with WhatsApp crawler). Added minimal og:title and og:description tags (required for WhatsApp large preview). Added "Copy image" button to share modal (copies PNG to clipboard).
